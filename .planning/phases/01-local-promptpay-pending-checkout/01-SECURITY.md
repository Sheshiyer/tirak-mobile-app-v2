...
- **Input state:** State B — both plans and both execution summaries exist; no prior security file existed.
...
| T-01-02 | Information Disclosure / Tampering | Cross-booking persisted payment state | mitigate | A different booking cannot replace a retained live or uncertain session; safe booking identity transitions invalidate the session version and clear prior charge/method state (`stores/payment-store.ts:130-175`). Booking submission is blocked while a retained financial session exists and rejects a refused store transition (`components/booking/steps/BookingSummaryStep.tsx:124-127`, `157-160`, `216-224`). Tests prove pending and indeterminate sessions cannot be replaced (`__tests__/payment-store.test.ts:198-235`). This intentionally supersedes unconditional form-reset deletion: form reset retains unresolved financial truth rather than leaking it into a new booking. | closed |
| T-01-03 | Information Disclosure | Cross-user payment state | mitigate | Central auth invalidation clears in-memory and persisted payment state together with credentials (`stores/auth-store.ts:55-70`); login, registration, demo login, user update, and token rehydration reset on identity change (`stores/auth-store.ts:100`, `160`, `298`, `324`, `347`). Tests cover logout, missing/corrupt credentials, storage failure, unauthorized events, identity transition, and late-response invalidation (`__tests__/payment-session-reset.test.ts:111-208`). | closed |
| T-01-04 | Tampering / Denial of Service | Charge creation and retry | mitigate | A module-level in-flight promise is returned before any second request, and the latch is set synchronously around the only client call (`stores/payment-store.ts:180-242`). Retry is limited to failed/expired attempts for the same booking and refuses live or terminal states (`stores/payment-store.ts:245-258`). UI controls independently refuse creating/closed states (`components/booking/steps/PaymentSelectionStep.tsx:216-239`, `336-351`). Tests prove one invocation for concurrent creation and prohibit unsafe retry (`__tests__/payment-store.test.ts:54-68`, `251-290`). | closed |
| T-01-05 | Spoofing / Tampering | Payment status and traveler-facing truth | mitigate | The decoder accepts only the frozen status pairs (`app/api/payment/payment.ts:68-97`, `110-147`), and store phases derive exclusively from a validated response or server booking status (`stores/payment-store.ts:67-93`, `203-233`). The store interface exposes no paid/success setter (`stores/payment-store.ts:35-48`), verified by `__tests__/payment-store.test.ts:323-328`. Pending UI renders `Payment pending` from decoded server fields (`components/booking/payment/PromptPayPendingCard.tsx:31-79`), while confirmation separates booking and payment state (`components/booking/steps/BookingConfirmationStep.tsx:99-147`, `214-230`). | closed |
...
|-------|----------------|-------|
| Focused security/behavior suites | PASS — 6/6 suites, 161/161 tests | Payment request/decoder, payment store, session isolation, capability predicate, traveler checkout, wizard flow |
...
|------------|---------------|--------|------|--------|
| 2026-09-12 | 11 | 11 | 0 | gsd-security-auditor |
